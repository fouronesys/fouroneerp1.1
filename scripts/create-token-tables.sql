-- =================================================================
-- Script para crear tablas de tokens y autenticación
-- Diseñado para deployment en plataformas externas
-- =================================================================
...
CREATE TABLE IF NOT EXISTS "session_tokens" (
    ...
    "session_data" TEXT NOT NULL, -- JSON string con información de sesión
...
-- 8. Tabla de tokens de dispositivos (para notificaciones push)
...
COMMENT ON TABLE login_attempts IS 'Registro de intentos de login para seguridad y análisis';
COMMENT ON TABLE session_tokens IS 'Tokens de sesión para autenticación persistente estilo WhatsApp';
COMMENT ON TABLE password_reset_tokens IS 'Tokens temporales para reseteo de contraseñas';
COMMENT ON TABLE api_tokens IS 'Tokens de API para integraciones y acceso programático';
COMMENT ON TABLE refresh_tokens IS 'Tokens de actualización para OAuth y JWT';
COMMENT ON TABLE invitation_tokens IS 'Tokens de invitación para registro en empresas';
COMMENT ON TABLE email_verification_tokens IS 'Tokens para verificación de direcciones de email';
COMMENT ON TABLE device_tokens IS 'Tokens de dispositivos para notificaciones push';

COMMENT ON FUNCTION cleanup_expired_tokens() IS 'Función para limpiar automáticamente tokens expirados';
...
DO $$
BEGIN
    RAISE NOTICE 'Script de tokens completado exitosamente';
    RAISE NOTICE 'Tablas creadas: login_attempts, session_tokens, password_reset_tokens, api_tokens, refresh_tokens, invitation_tokens, email_verification_tokens, device_tokens';
    RAISE NOTICE 'Función de limpieza: cleanup_expired_tokens() disponible';
    RAISE NOTICE 'Para ejecutar limpieza: SELECT cleanup_expired_tokens();';
END $$;